def whoosh(folder):
    ...
    import os
    schema = Schema(title=TEXT(stored=True), typ=ID(stored) id=ID(stored=True), content=TEXT)
    ...
    def search(**a):
        ...
        with ix.searcher() as searcher:
            queries = [QueryParser(key, ix.schema).parse(unicode(value)) for key,value in a.items()]
